...
**Kết quả: PASS ngay lần đầu** (0 thread rò rỉ) — xác nhận **thật** (không chỉ đọc code) rằng cơ chế dọn dẹp đã có từ trước xử lý đúng cho trục "nhiều phòng qua thời gian", không chỉ trục "nhiều peer cùng lúc" đã kiểm tra trước đó. Không cần sửa code — ghi nhận là 1 khía cạnh đã kiểm chứng an toàn, không phải bug mới.
...
39 test đã tự chạy bằng IntelliJ và **PASS**:
